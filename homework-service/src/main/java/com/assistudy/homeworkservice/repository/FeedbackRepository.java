...
public interface FeedbackRepository extends JpaRepository<Feedback, Long> {
    ...
    @Query("SELECT f FROM Feedback f JOIN FETCH f.homework h WHERE h.roomId = :roomId AND f.userId = :userId AND DATE(f.date) = :date")
    List<Feedback> findByRoomIdAndDateAndUserId(@Param("roomId") Long roomId,
                                                @Param("date") LocalDate date,
                                                @Param("userId") Long userId);

    // 특정 방, 특정 날짜의 모든 피드백 조회
    @Query("SELECT f FROM Feedback f JOIN FETCH f.homework h WHERE h.roomId = :roomId AND DATE(f.date) = :date")
    List<Feedback> findByRoomIdAndDate(@Param("roomId") Long roomId, @Param("date") LocalDate date);

    // 특정 방, 특정 사용자의 모든 피드백 조회
    @Query("SELECT f FROM Feedback f JOIN FETCH f.homework h WHERE h.roomId = :roomId AND f.userId = :userId")
