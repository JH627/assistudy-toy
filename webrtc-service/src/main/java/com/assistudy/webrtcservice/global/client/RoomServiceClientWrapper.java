 ...
 * room 조회는 존재확인/host검증처럼 인가에 직결되는 호출이라 닉네임 표시와 달리 완화할 수 없으므로,
 * 회로가 열리면 요청 자체를 실패시키는 fail-closed로 설계했다(500 반환).
 */
@Slf4j
@Primary
@Service
...
public class RoomServiceClientWrapper implements RoomServiceClient {
    ...
    @Override
    @CircuitBreaker(name = CB_NAME, fallbackMethod = "countParticipantsFallback")
    public ApiResponse<Integer> countParticipants(Long roomId) {
        return feignClient.countParticipants(roomId);
    }
    ...
    private ApiResponse<Integer> countParticipantsFallback(Long roomId, Throwable t) {
        log.warn("[CB] countParticipants fallback (fail-closed) - roomId={}, cause={}", roomId, t.toString());
        throw new CustomException(GeneralErrorCode.INTERNAL_SERVER_ERROR);
    }
}
